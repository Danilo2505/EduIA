...
export type ContentRow = {
  ...
  category: Category; tag: string | null; emoji: string | null;
...
export const Content = {
  ...
  async create(userId: number, title: string, body: string, category: Category, tag?: string, emoji?: string) {
  ...
  async update(userId: number, id: number, title: string, body: string, tag?: string, emoji?: string) {
    const [r] = await pool.query(
  ...
  async remove(userId: number, id: number) {
    const [r] = await pool.query('DELETE FROM contents WHERE id=? AND user_id=?', [id, userId]);
    return (r as any).affectedRows > 0;
  },
};
